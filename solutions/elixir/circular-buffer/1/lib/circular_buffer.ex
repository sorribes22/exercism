defmodule CircularBuffer do
  ...
  def is_full?(circular_buffer), do: circular_buffer.count == circular_buffer.size
  def b_write(circular_buffer, item, overwrite? \\ false) do
    is_full? = is_full?(circular_buffer)

    if is_full? and not overwrite? do
      {:error, :full}
    else
      new_first = if is_full?, do: rem(circular_buffer.first + 1, circular_buffer.size), else: circular_buffer.first
      new_count = if is_full?, do: circular_buffer.count, else: circular_buffer.count + 1
